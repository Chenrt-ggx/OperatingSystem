...
void unmap_block(u_int blockno)
{
	...
	if (syscall_mem_unmap(0, diskaddr(blockno)))
		user_panic("failed to unmap in unmap block");
...
int block_is_free(u_int blockno)
{
	if (super == 0 || blockno >= super->s_nblocks)
		return 0;
	if (bitmap[blockno >> 5] & (1 << (blockno & 31)))
		return 1;
	return 0;
}
...
void free_block(u_int blockno)
{
	...
	if (blockno < 1 || (super && blockno >= super->s_nblocks))
		return;
...
int dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	nblock = dir->f_size / BY2BLK + (dir->f_size % BY2BLK != 0);

	for (i = 0; i < nblock; i++)
	{
		// Step 2: Read the i'th block of the dir.
		// Hint: Use file_get_block.
		if ((r = file_get_block(dir, i, (void *)&blk)) < 0)
			return r;
		...
		for (j = 0; j < FILE2BLK; j++, blk++)
			if (!strcmp((char *)blk->f_name, name))
			{
				blk->f_dir = dir;
				*file = blk;
				return 0;
