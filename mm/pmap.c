...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	...
	pgtable_entry = (Pte *)KADDR(PTE_ADDR(*pgdir_entryp));
	return pgtable_entry + PTX(va);
}
...
void page_init(void)
{
	...
	int i;
	for (i = 0; i < PADDR(freemem) / BY2PG; (pages + i++)->pp_ref = 1);

	/* Step 4: Mark the other memory as free. */
	for (i = PADDR(freemem) / BY2PG; i < npage; (pages + i++)->pp_ref = 0)
...
int page_alloc(struct Page **pp)
{
	...
	// Hint: use `bzero`.
	bzero((void *)page2kva(*pp = ppage_temp), BY2PG);
	return 0;
}
