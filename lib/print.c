...
void lp_Print(void (*output)(void *, char *, int), void *arg, char *fmt, va_list ap)
{
	...
	long int num;
	char c, *s, padc, buf[LP_MAX_BUF];
	int longFlag, width, prec, ladjust, length;
	...
	for (; *fmt;)
	{
		...
		for (width = 0; IsDigit(*fmt); fmt++)
			width = (width << 1) + (width << 3) + ((*fmt) ^ 48);
		if (*fmt == '.')
			fmt++;
		for (prec = 0; IsDigit(*fmt); fmt++)
			prec = (prec << 1) + (prec << 3) + ((*fmt) ^ 48);
		if (*fmt == 'l')
			longFlag = 1, fmt++;
		else
			longFlag = 0;
		...
		case 'D':
			num = longFlag ? va_arg(ap, long int) : va_arg(ap, int);
			length = PrintNum(buf, num < 0 ? -num : num, 10, num < 0, width, ladjust, padc, 0);
			OUTPUT(arg, buf, length);
			break;
...
int PrintNum(char *buf, unsigned long u, int base, int negFlag, int length, int ladjust, char padc, int upcase)
{
	...
	*/
	...
	int begin = 0;
	int end = ladjust ? (actualLength - 1) : (length - 1);
	while (end > begin)
	{
		char tmp = buf[begin];
		buf[begin] = buf[end];
