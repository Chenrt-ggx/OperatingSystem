...
int envid2env(u_int envid, struct Env **penv, int checkperm)
{
	...
	if (checkperm && e - curenv && e->env_id - curenv->env_id && e->env_parent_id - curenv->env_id)
	{
		*penv = NULL;
		return -E_BAD_ENV;
	}
...
static int load_icode_mapper(u_long va, u_int32_t sgsize, u_char *bin, u_int32_t bin_size, void *user_data)
{
	...
	u_char *ptr;
	struct Page *p = NULL;
	u_long i, j, cur, tot, offset, va_align;
	...
	for (i = cur = 0, va_align = ROUNDDOWN(va, BY2PG), offset = va - va_align,
		tot = (ROUND(va + sgsize, BY2PG) - va_align) / BY2PG;
		 i < tot; i++)
	{
		if (!i && offset)
		{
			p = page_lookup(env->env_pgdir, va_align, NULL);
			if (!p && (r = page_alloc(&p) < 0))
				return r;
		}
		else if ((r = page_alloc(&p)) < 0)
			return r;
		page_insert(env->env_pgdir, p, va_align + (i << PGSHIFT), PTE_R);
		for (j = 0, ptr = (u_char *)page2kva(p); j < BY2PG; j++)
			if (cur < offset)
				cur++;
			else if (cur < bin_size + offset)
				*(ptr + j) = bin[cur++ - offset];
			else if (cur < sgsize + offset)
				*(ptr + j) = 0, cur++;
			else
				break;
	}
...
static int load_icode_mapper(u_long va, u_int32_t sgsize, u_char *bin, u_int32_t bin_size, void *user_data)
{
	int r;
	struct Page *p = NULL;
	u_long i = 0, size, offset;
	struct Env *env = (struct Env *)user_data;

	// Step 1: load all content of bin into memory.
	if ((offset = va - ROUNDDOWN(va, BY2PG)))
	{
		if (!(p = page_lookup(env->env_pgdir, va, NULL)))
		{
			if ((r = page_alloc(&p)) < 0)
				return r;
			page_insert(env->env_pgdir, p, va, PTE_R);
		}
		i = size = MIN(bin_size, BY2PG - offset);
	...
	// Step 2: alloc pages to reach `sgsize` when `bin_size` < `sgsize`.
	// hint: variable `i` has the value of `bin_size` now!
	if ((offset = va + i - ROUNDDOWN(va + i, BY2PG)))
	{
		if (!(p = page_lookup(env->env_pgdir, va + i, NULL)))
		{
			if ((r = page_alloc(&p)) < 0)
				return r;
			page_insert(env->env_pgdir, p, va + i, PTE_R);
		}
		i += (size = MIN(sgsize - i, BY2PG - offset));
...
static void load_icode(struct Env *e, u_char *binary, u_int size)
{
	...
	if (page_insert(e->env_pgdir, p, USTACKTOP - BY2PG, perm))
		return;
